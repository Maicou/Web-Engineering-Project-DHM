    ...
    <body>
        ...
        <section id="main">
            <nav class="nav3">
                ...
                $stmt = $conn->prepare("SELECT * FROM tenant WHERE tenant.Accommodation_id >=9;");
                $stmt->execute();
                $totalsp;
                $tenantNumber = 0;
                $amount = 0;
                ...
                $stmt = ("SELECT * FROM tenant JOIN incomings WHERE incomings.Tenant_tid = tenant.tid AND incomings.Incometypes_id = 1 and tenant.Accommodation_id >=9;");
                foreach ($conn->query($stmt) as $row) {
                    $amount += $row['amount'];
                }
                $stmt = ("SELECT * FROM tenant JOIN incomings WHERE incomings.Tenant_tid = tenant.tid AND incomings.Incometypes_id = 2 and tenant.Accommodation_id >=9;");
                foreach ($conn->query($stmt) as $row) {
                    $amount += $row['amount'];
                }
                $stmt = ("SELECT totalLivingSpace FROM `building` WHERE building.id = '2';");
                foreach ($conn->query($stmt) as $row) {
            ...
            <div class="content">                                           
                <!--the content-->
                <table>
                    <th><b><h2>Information: Hauenstein-Gebäude</h2></b></th>
                    <th></th>
                    <tr>
                        <td>Strasse:</td> <td>Hauensteinstr. 7</td>
                    </tr>
                    <tr>
